...
# -*- coding: utf-8 -*-
"""
SSE streaming test for the OpenClaw Agent ↔ A2A Daemon Engine integration.
...
This script tests the real-time streaming path:
    ...
    Client (this script)
      ...
      → A2ADaemonExecutor → Phase 10 bridge → OpenClawAgentHandler
...
chunks as they arrive in real-time.  It also verifies the OpenClaw
chat/completions streaming endpoint directly for comparison.
...
Prerequisites:
    ...
    - PostgreSQL container running with openclaw-agent registered

Usage:
    python silvaengine_gateway/tests/test_openclaw_sse_live.py

    # With custom prompt
    python silvaengine_gateway/tests/test_openclaw_sse_live.py --prompt "Write a haiku about AI"
...
def run_sse_test(gateway_url, openclaw_url, openclaw_key, token, endpoint_id, part_id, prompt):
    _section("OpenClaw Agent SSE Streaming Test")
    ...
    task_id = f"openclaw-sse-{uuid.uuid4().hex[:8]}"
    ...
    body = {
        ...
        "params": {
            ...
            "metadata": {
                ...
                "stream": True,
                "task_data": {"task_id": task_id, "task_type": "openclaw_sse_test"},
            },
        },
        "id": "openclaw-sse-test-001",
    }
    ...
    # 5. Check OpenClaw API directly for comparison (streaming chat/completions)
    _section("Direct OpenClaw API Verification (streaming chat/completions)")
    ...
    if openclaw_r.status_code == 200:
        _pass("OpenClaw streaming chat/completions", f"HTTP {openclaw_r.status_code}")
        openclaw_text = ""
                ...
                try:
                    ...
                    if choices:
                        ...
                        if delta_content:
                            ...
                            print(f"  {_C.YELLOW}openclaw chunk{_C.RESET}: {delta_content}", end="", flush=True)
                except json.JSONDecodeError:
                    pass
        print()
        if openclaw_text:
            _pass("OpenClaw streaming text", f"'{openclaw_text[:120]}'")
        else:
            _fail("OpenClaw streaming text", "No text received from direct streaming call")
    else:
        _fail("OpenClaw streaming chat/completions", f"HTTP {openclaw_r.status_code}")
...
def main():
    parser = argparse.ArgumentParser(
        description="SSE streaming test: OpenClaw Agent through SilvaEngine Gateway"
    )
    ...
    run_sse_test(gateway_url, openclaw_url, openclaw_key, token, endpoint_id, part_id, args.prompt)
