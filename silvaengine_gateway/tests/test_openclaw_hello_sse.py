...
# -*- coding: utf-8 -*-
"""
One-shot "say hello from OpenClaw" through the A2A Daemon Engine
via the SilvaEngine Gateway with SSE streaming.
...
Usage:
    python silvaengine_gateway/tests/test_openclaw_hello_sse.py
...
PROMPT = "Say hello from OpenClaw through A2A. Reply in one short sentence."
...
def main():
    ...
    try:
        ...
        if ocr.status_code != 200:
            print(f"OpenClaw Gateway not healthy: HTTP {ocr.status_code}")
            return 1
    except Exception as e:
        print(f"Cannot reach OpenClaw at {openclaw_url}: {e}")
        return 1
