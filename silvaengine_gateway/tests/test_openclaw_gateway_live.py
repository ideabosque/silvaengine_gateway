...
# -*- coding: utf-8 -*-
"""
Live E2E test for the OpenClaw Agent ↔ A2A Daemon Engine integration
...
Full request path:
    ...
    Client
      ...
      → Phase 10 bridge → OpenClawAgentHandler
      → OpenClaw Gateway (http://127.0.0.1:18789)
      → JSON-RPC response back through gateway

Test sequence (mirrors test_hermes_gateway_live.py):

    1. Probe OpenClaw /v1/models
    2. Probe gateway health and A2A route availability
    3. Register or verify the openclaw-agent fixture via gateway GraphQL
    4. Non-streaming message/send through gateway
    5. Compatibility message/send (different prompt)
    6. Cancel a long-running task (best-effort — OpenClaw has no server-side stop)
    7. Failure case — unknown agent + wrong OpenClaw key
...
Prerequisites:
    ...
    - OpenClaw env vars in the gateway .env:
      OPENCLAW_API_URL, OPENCLAW_API_KEY, OPENCLAW_AGENT_ID,
      OPENCLAW_AGENT_SELECTOR, OPENCLAW_STREAM_TIMEOUT

Usage:
    # Set env var to enable live tests (pytest mode)
    A2A_RUN_LIVE_OPENCLAW_TESTS=1 python -m pytest silvaengine_gateway/tests/test_openclaw_gateway_live.py -v

    # Or run directly (auto-reads gateway .env, generates token)
    python silvaengine_gateway/tests/test_openclaw_gateway_live.py

    # With explicit args
    python silvaengine_gateway/tests/test_openclaw_gateway_live.py \
        --gateway-url http://127.0.0.1:8765 \
        --openclaw-url http://127.0.0.1:18789 \
        --openclaw-key YOUR_TOKEN
...
pytestmark = pytest.mark.skipif(
    os.getenv("A2A_RUN_LIVE_OPENCLAW_TESTS", "").lower() not in {"1", "true", "yes"},
    reason="Live OpenClaw gateway E2E tests require A2A_RUN_LIVE_OPENCLAW_TESTS=1 "
    "plus running OpenClaw Gateway and SilvaEngine Gateway.",
)
...
# ---------------------------------------------------------------------------
# OpenClaw probe helpers
# ---------------------------------------------------------------------------

def _openclaw_get(openclaw_url, openclaw_key, path):
    try:
        r = requests.get(
            f"{openclaw_url}{path}",
...
def openclaw_models_ok(openclaw_url, openclaw_key):
    """Check that OpenClaw /v1/models returns model list with openclaw agents."""
    r = _openclaw_get(openclaw_url, openclaw_key, "/v1/models")
    if r is None or r.status_code != 200:
        return False
    data = r.json().get("data", [])
    model_ids = [m.get("id") for m in data]
    return any("openclaw" in mid for mid in model_ids if mid)
...
def register_openclaw_agent(gateway_url, token, endpoint_id, part_id):
    mutation = """
        mutation RegisterOpenClawAgent(
            $endpointId: String!
            $partId: String!
            $agentId: String
            $agentName: String!
...
# ---------------------------------------------------------------------------
# Test 1: OpenClaw health and models
# ---------------------------------------------------------------------------

def test_01_openclaw_health(ctx):
    _section("TEST 1: OpenClaw Gateway Health & Models")
    assert openclaw_models_ok(ctx["openclaw_url"], ctx["openclaw_key"]), \
        f"OpenClaw /v1/models failed at {ctx['openclaw_url']}"
    _pass("OpenClaw /v1/models", "openclaw models present")
    # Also verify direct chat completions endpoint works
    r = requests.post(
        f"{ctx['openclaw_url']}/v1/chat/completions",
        json={
            "model": "openclaw/main",
        ...
        headers={
            "Authorization": f"Bearer {ctx['openclaw_key']}",
            "Content-Type": "application/json",
        },
        timeout=30,
    )
    assert r.status_code == 200, f"OpenClaw chat/completions failed: HTTP {r.status_code}"
    _pass("OpenClaw /v1/chat/completions", "direct chat works")
...
# ---------------------------------------------------------------------------
# Test 3: Register openclaw-agent fixture
# ---------------------------------------------------------------------------

def test_03_register_agent(ctx):
    _section("TEST 3: Register/Verify OpenClaw Agent Fixture")
    result = register_openclaw_agent(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"])
    if result:
        _pass("Agent Registration", f"agent_id={result.get('agentId', OPENCLAW_AGENT_ID)}")
    else:
...
def test_04_non_streaming(ctx):
    ...
    r = send_a2a(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"],
                "message/send", params, "openclaw-e2e-send-001")
...
def test_05_compat(ctx):
    ...
    params = {
        "message": {
            "role": "user",
            "parts": [{"text": "Confirm you are OpenClaw Agent. Reply in one sentence."}],
        },
        "metadata": {
            "operation": "message_response",
            "agent_uuid": OPENCLAW_AGENT_ID,
    ...
    r = send_a2a(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"],
                "message/send", params, "openclaw-e2e-compat-001")
    assert r.status_code == 200, f"HTTP {r.status_code}: {r.text[:300]}"
    body = r.json()
...
def test_06_cancel(ctx):
    _section("TEST 6: CancelTask via Gateway (best-effort — OpenClaw has no server-side stop)")
    task_id = f"openclaw-cancel-{uuid.uuid4().hex[:8]}"
    import threading

    stream_result = {"response": None, "error": None}
    # Use message_response (not task_execution) because the A2A SDK v2
    # on_message_send expects a single Message object.
    params = {
        "message": {
            "role": "user",
        ...
        "metadata": {
            "operation": "message_response",
            "agent_uuid": OPENCLAW_AGENT_ID,
            "task_data": {"task_id": task_id, "task_type": "openclaw_e2e_cancel"},
        },
    }
    ...
    def _bg():
        try:
            r = send_a2a(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"],
                        "message/send", params, "openclaw-e2e-cancel-stream", timeout=120)
    ...
    r = send_a2a(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"],
                "tasks/cancel", cancel_params, "openclaw-e2e-cancel-001", timeout=30)
    assert r.status_code == 200, f"HTTP {r.status_code}: {r.text[:300]}"
    body = r.json()
    if body.get("error"):
...
# ---------------------------------------------------------------------------
# Test 7: Failure case — unknown agent + wrong OpenClaw key
# ---------------------------------------------------------------------------

def test_07_failure(ctx):
    _section("TEST 7: Failure Case — Unknown Agent + Wrong OpenClaw Key")

    # 7a: Unknown agent through the full pipeline
    params = {
        "message": {
            "role": "user",
    ...
    r = send_a2a(ctx["gateway_url"], ctx["token"], ctx["endpoint_id"], ctx["part_id"],
                "message/send", params, "openclaw-e2e-fail-001", timeout=60)
    assert r.status_code == 200, f"HTTP {r.status_code}: {r.text[:300]}"
    body = r.json()
    text = extract_text(body.get("result", {}))
    error = body.get("error")
    ...
    # 7b: Verify wrong OpenClaw key is rejected at the API level
    wrong_r = _openclaw_get(ctx["openclaw_url"], "wrong-key-12345", "/v1/models")
    ...
    _pass("OpenClaw wrong-key rejection", f"HTTP {wrong_r.status_code}")

    # 7c: Verify correct key still works (sanity check)
    ok_r = _openclaw_get(ctx["openclaw_url"], ctx["openclaw_key"], "/v1/models")
    assert ok_r is not None and ok_r.status_code == 200, \
        f"Expected 200 with correct key, got {ok_r.status_code if ok_r else 'None'}"
    _pass("OpenClaw correct-key accepted", f"HTTP {ok_r.status_code}")
...
def build_context(args):
    ...
    openclaw_url = args.openclaw_url or env.get("OPENCLAW_API_URL", "http://127.0.0.1:18789")
    openclaw_key = args.openclaw_key or env.get("OPENCLAW_API_KEY", "")
...
def run_all(ctx):
    results = []
    tests = [
        ("OpenClaw Health & Models", lambda: test_01_openclaw_health(ctx)),
        ("Gateway Health & A2A", lambda: test_02_gateway_health(ctx)),
        ("Register OpenClaw Agent", lambda: test_03_register_agent(ctx)),
...
def main():
    parser = argparse.ArgumentParser(
        description="Live E2E test: OpenClaw Agent ↔ A2A Daemon via SilvaEngine Gateway"
    )
    ...
    parser.add_argument("--openclaw-url", default=None,
                        help="OpenClaw Gateway base URL (default: from gateway .env)")
    parser.add_argument("--openclaw-key", default=None,
                        help="OpenClaw API key (default: from gateway .env)")
    ...
    _info(f"OpenClaw:  {ctx['openclaw_url']}")
