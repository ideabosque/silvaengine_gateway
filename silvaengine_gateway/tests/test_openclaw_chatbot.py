...
# -*- coding: utf-8 -*-
"""
Interactive chatbot test for OpenClaw Agent through the A2A Daemon Engine
via the SilvaEngine Gateway.

Opens an SSE connection for real-time streaming and lets you chat
with the OpenClaw Agent interactively.  Each message you type is sent
through the full pipeline:

    You (stdin)
      → POST /{endpoint_id}/a2a  (message/send)
      → SilvaEngine Gateway → A2ADaemonExecutor → Phase 10 bridge
      → OpenClawAgentHandler → OpenClaw Gateway (POST /v1/chat/completions stream:true → SSE)
...
Usage:
    python silvaengine_gateway/tests/test_openclaw_chatbot.py

    # With a custom system prompt
    python silvaengine_gateway/tests/test_openclaw_chatbot.py --system "You are a pirate"
